...
# Filtering for 2026 only
data <- data %>%
  filter(year == 2026)


#  defining storm window
storm_start <- as.Date("2026-01-24")
storm_end   <- as.Date("2026-01-27")
...
ggplot(daily_counts, aes(x = date, y = count, color = storm_period)) +
  ...
  geom_point(size = 2) +
  labs(
    title = "Dog Droppings Complaints Before, During, and After January 2026 Snow Storm",
    x = "Date",
...
ggplot(summary_table, aes(x = storm_period, y = total_complaints, fill = storm_period)) +
  ...
  labs(
    title = "Total Dog Droppings Complaints per Storm Period (January 2026)",
    x = "Storm Period",
    y = "Total Complaints"
  ) +
  theme_minimal() +
